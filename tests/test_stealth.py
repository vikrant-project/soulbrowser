"""Stealth detection tests for soulbrowser.
...
PROXY = os.environ.get("SOULBROWSER_TEST_PROXY")
...
class TestIssueRegressions:
    ...
    def test_add_init_script_without_proxy(self, browser):
        ...
        """
        page = browser.new_page()
        page.add_init_script("window.__soultest = 42;")
        page.goto("https://example.com", timeout=30000)
        val = page.evaluate("window.__soultest")
        page.close()
    ...
    def test_add_init_script_with_proxy(self, browser):
        ...
        proxy = os.environ.get("SOULBROWSER_TEST_PROXY")
        if not proxy:
            pytest.skip("SOULBROWSER_TEST_PROXY not set")

        ctx = browser.new_context(proxy={"server": proxy})
        page = ctx.new_page()
        page.add_init_script("window.__soultest = 99;")
        try:
