...
from soulbrowser.browser import (
    _is_socks_proxy,
    _parse_proxy_url,
...
class TestBuildProxyKwargs:
    ...
    @patch("soulbrowser.config.get_chromium_version", return_value="146.0.7680.177.5")
    @patch("soulbrowser.config.get_platform_tag", return_value="linux-x64")
...
class TestMaybeResolveGeoip:
    @patch("soulbrowser.geoip.resolve_proxy_geo_with_ip", return_value=("America/New_York", "en-US", "1.2.3.4"))
    ...
    @patch("soulbrowser.geoip.resolve_proxy_geo_with_ip", return_value=("Europe/London", "en-GB", "5.6.7.8"))
    ...
    @patch("soulbrowser.geoip.resolve_proxy_geo_with_ip", return_value=("Asia/Tokyo", "ja-JP", "9.8.7.6"))
    ...
    @patch("soulbrowser.geoip.resolve_proxy_geo_with_ip", return_value=("Europe/Berlin", "de-DE", "5.6.7.8"))
    ...
    @patch("soulbrowser.geoip.resolve_proxy_geo_with_ip", return_value=("Europe/London", "en-GB", "1.1.1.1"))
...
class TestResolveProxyConfig:
    ...
    def test_socks5_string_logs_info_when_reencoding(self, caplog):
        ...
        import logging
        with caplog.at_level(logging.INFO, logger="soulbrowser"):
    ...
    def test_socks5_string_malformed_port_passes_through(self, caplog):
        ...
        import logging
        with caplog.at_level(logging.WARNING, logger="soulbrowser"):
    ...
    @patch("soulbrowser.config.get_chromium_version", return_value="146.0.7680.177.5")
    @patch("soulbrowser.config.get_platform_tag", return_value="windows-x64")
    def test_http_string_with_creds_on_windows(self, *_):
        kwargs, args = _resolve_proxy_config("http://user:pass@proxy:8080")
        assert kwargs == {}
        assert args == ["--proxy-server=http://user:pass@proxy:8080"]

    @patch("soulbrowser.config.get_chromium_version", return_value="146.0.7680.177.3")
    @patch("soulbrowser.config.get_platform_tag", return_value="linux-x64")
    ...
    @patch("soulbrowser.config.get_platform_tag", return_value="darwin-arm64")
    ...
    @patch("soulbrowser.config.get_platform_tag", return_value="linux-arm64")
