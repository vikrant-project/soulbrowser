...
from soulbrowser.config import DEFAULT_VIEWPORT
...
@patch("soulbrowser.browser.ensure_binary", return_value="/fake/chrome")
@patch("soulbrowser.browser.maybe_resolve_geoip", return_value=(None, None, None))
def test_persistent_context_args_built(_mock_geoip, _mock_bin):
    ...
    with patch("playwright.sync_api.sync_playwright", return_value=pw_cm):
        from soulbrowser.browser import launch_persistent_context
...
@patch("soulbrowser.browser.maybe_resolve_geoip", return_value=("Europe/Berlin", "de-DE", "5.6.7.8"))
@patch("soulbrowser.browser.ensure_binary", return_value="/fake/chrome")
...
async def test_persistent_context_async_args_built(_mock_geoip, _mock_bin):
    ...
    with patch("playwright.async_api.async_playwright", return_value=pw_cm):
        from soulbrowser.browser import launch_persistent_context_async
