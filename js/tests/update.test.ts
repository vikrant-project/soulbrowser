...
describe("download URL", () => {
  it("uses chromium-v prefix and soulbrowser repo", () => {
...
describe("latest version (platform-aware)", () => {
  const platformTarball = `soulbrowser-${getPlatformTag()}.tar.gz`;

  function makeAssets(platforms: string[]) {
    return platforms.map((p) => ({ name: `soulbrowser-${p}.tar.gz` }));
  }
...
describe("wrapper update check", () => {
  beforeEach(() => {
    resetWrapperUpdateChecked();
    delete process.env.SOULBROWSER_AUTO_UPDATE;
    delete process.env.SOULBROWSER_DOWNLOAD_URL;
  });
  ...
  it("disabled by SOULBROWSER_AUTO_UPDATE=false", async () => {
    process.env.SOULBROWSER_AUTO_UPDATE = "false";
  ...
  it("disabled by SOULBROWSER_DOWNLOAD_URL", async () => {
    process.env.SOULBROWSER_DOWNLOAD_URL = "https://mirror.example.com";
    const spy = vi.spyOn(globalThis, "fetch");
...
describe("parseChecksums", () => {
  ...
  it("parses standard SHA256SUMS format", () => {
    const text = [
      `${HASH_A}  soulbrowser-linux-x64.tar.gz`,
      `${HASH_B}  soulbrowser-darwin-arm64.tar.gz`,
    ].join("\n");
    const result = parseChecksums(text);
    expect(result.get("soulbrowser-linux-x64.tar.gz")).toBe(HASH_A);
    expect(result.get("soulbrowser-darwin-arm64.tar.gz")).toBe(HASH_B);
  });

  it("handles binary-mode asterisk prefix", () => {
    const text = `${HASH_A} *soulbrowser-linux-x64.tar.gz`;
    const result = parseChecksums(text);
    expect(result.has("soulbrowser-linux-x64.tar.gz")).toBe(true);
  });
...
describe("download fallback", () => {
  ...
  it("checksum fetch falls back to GitHub on primary 429", async () => {
    ...
    const checksumText = `${HASH}  soulbrowser-${getPlatformTag()}.tar.gz`;
    ...
    vi.spyOn(globalThis, "fetch").mockImplementation(async (input) => {
      ...
      if (url.includes("soulbrowser.dev")) {
        return {
          ok: false,
          status: 429,
    ...
    expect(
      result!.has(`soulbrowser-${getPlatformTag()}.tar.gz`)
    ).toBe(true);
  });
...
describe("effective version", () => {
  it("returns platform version when no marker exists", () => {
    const orig = process.env.SOULBROWSER_CACHE_DIR;
    process.env.SOULBROWSER_CACHE_DIR = `/tmp/soulbrowser-test-${Date.now()}`;
    try {
      expect(getEffectiveVersion()).toBe(getChromiumVersion());
    } finally {
      if (orig) process.env.SOULBROWSER_CACHE_DIR = orig;
      else delete process.env.SOULBROWSER_CACHE_DIR;
    }
  });
});
...
describe("clearCache", () => {
  it("does not throw when cache dir missing", () => {
    const orig = process.env.SOULBROWSER_CACHE_DIR;
    process.env.SOULBROWSER_CACHE_DIR = "/tmp/soulbrowser-test-nonexistent";
    expect(() => clearCache()).not.toThrow();
    if (orig) {
      process.env.SOULBROWSER_CACHE_DIR = orig;
    } else {
