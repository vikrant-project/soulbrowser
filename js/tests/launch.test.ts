...
describe("binaryInfo", () => {
  it("returns correct structure", () => {
    ...
    try {
      ...
      expect(info.cacheDir).toContain("soulbrowser");
    } finally {
...
describe("composable Playwright launch helpers", () => {
  const origBinaryPath = process.env.SOULBROWSER_BINARY_PATH;

  beforeEach(() => {
    process.env.SOULBROWSER_BINARY_PATH = "/fake/chrome";
    vi.resetModules();
  });
  ...
  afterEach(() => {
    ...
    vi.resetModules();
    if (origBinaryPath) {
      process.env.SOULBROWSER_BINARY_PATH = origBinaryPath;
    } else {
...
//   SOULBROWSER_BINARY_PATH=/path/to/chrome npm test
describe.skipIf(!process.env.SOULBROWSER_BINARY_PATH)(
  "launch (integration)",
  () => {
...
describe("launchContext (unit)", () => {
  ...
  const origEnv = process.env.SOULBROWSER_BINARY_PATH;
  ...
  afterEach(() => {
    ...
    vi.resetModules();
    if (origEnv) {
      process.env.SOULBROWSER_BINARY_PATH = origEnv;
    } else {
      delete process.env.SOULBROWSER_BINARY_PATH;
    }
  });
