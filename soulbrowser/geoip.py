...
Optional feature — requires ``geoip2`` package::
    ...
    pip install soulbrowser[geoip]

Downloads GeoLite2-City.mmdb (~70 MB) on first use, caches in
``~/.soulbrowser/geoip/``.  Background re-download after 30 days.
"""
...
GEOIP_TIMEOUT_ENV = "SOULBROWSER_GEOIP_TIMEOUT_SECONDS"
...
def _resolve_exit_ip(proxy_url: str, timeout: float | None = None) -> str | None:
    ...
    for url in _IP_ECHO_URLS:
        try:
            ...
            logger.warning(
                "SOCKS5 proxy requires socksio: pip install soulbrowser[geoip]"
            )
            return None
        except Exception:
            continue
