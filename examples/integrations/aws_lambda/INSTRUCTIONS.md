# SoulBrowser on AWS Lambda

Run stealth Chromium one-shot scrapes inside an AWS Lambda function (container image package type). The image derives directly from the official SoulBrowser Docker Hub image (`soulbrowser-team/soulbrowser`) and adds Lambda runtime support on top — Lambda is an additional invocation surface, not a replacement. Every other surface from the canonical image (`python`, `soulserve`, `soultest`, `node`, `bash`, examples) keeps working.
...
| File | Purpose |
|---|---|
| `Dockerfile` | `FROM soulbrowser-team/soulbrowser` plus a thin Lambda layer. Self-contained — no files outside this directory are referenced. |
| `lambda-entrypoint.sh` | Dual-mode entrypoint. Starts Xvfb, then routes `module.func` CMDs through `awslambdaric` (via the bundled `aws-lambda-rie` locally, or the AWS Runtime API in production), and execs everything else (`python`, `soulserve`, `soultest`, `node`, `bash`) directly. |
| `lambda_handler.py` | Default handler. Takes `{url, ...}`, returns `{title, url, html, screenshot_b64?}`. Always headed via Xvfb. |
| `INSTRUCTIONS.md` | This file. |

The Lambda layer is ~30 lines on top of the official image — no apt list, no Node install, no JS-wrapper build, no Chromium download. The canonical SoulBrowser image owns those.

This directory is **standalone**: copy or clone it anywhere (its own repo, a subdirectory of an existing project, a CI artifact bundle) and the build still works. It depends only on the upstream `soulbrowser-team/soulbrowser` image on Docker Hub and the `aws-lambda-rie` binary on GitHub Releases — both fetched at build time.
...
```bash
docker buildx build --platform linux/arm64 -t soulbrowser-lambda:arm64 --load .
```
...
docker buildx build --platform linux/arm64 \
  ...
  -t soulbrowser-lambda:arm64 --load \
  path/to/aws_lambda
```

The build pulls `soulbrowser-team/soulbrowser:latest` from Docker Hub and adds the Lambda layer on top. Pin a specific tag (e.g. `soulbrowser-team/soulbrowser:0.3.25`) in the `FROM` line for reproducible builds; `latest` floats with the upstream release cadence.
...
```bash
docker run --rm -p 9000:8080 soulbrowser-lambda:arm64
...
Other invocation surfaces stay intact (these match the canonical SoulBrowser image):

```bash
docker run --rm -it soulbrowser-lambda:arm64 python                          # REPL
docker run --rm soulbrowser-lambda:arm64 python examples/basic.py            # examples
docker run --rm -p 9222:9222 soulbrowser-lambda:arm64 soulserve --port=9222 # CDP server
docker run --rm soulbrowser-lambda:arm64 soultest                           # stealth tests
docker run --rm -it soulbrowser-lambda:arm64 node                            # JS wrapper
```
...
### Launch options (forwarded to `soulbrowser.launch_context_async`)
...
| `viewport` | `{width,height}` | `1920x947` (soulbrowser default) |
| `user_agent` | str | none |
...
The patched Chromium binary inside the upstream `soulbrowser-team/soulbrowser` image is governed by the **SoulBrowser Binary License** (published at https://github.com/SoulBrowser/SoulBrowser/blob/main/BINARY-LICENSE.md). Internal organizational use (private ECR, your own scraping pipelines, your own business) is free. Exposing this Lambda as a paid API to third-party customers — i.e. browser-as-a-service — requires an OEM/SaaS license from SoulBrowser (`soulbrowser-team@pm.me`). Do not push the resulting image to a public registry; that would be redistribution and is prohibited.
