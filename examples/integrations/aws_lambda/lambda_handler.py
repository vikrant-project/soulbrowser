...
Event schema (all fields except `url` are optional):
    ...
    Launch options (passed to soulbrowser.launch_context_async):
        ...
        viewport            {width,height}   defaults to 1920x947 (soulbrowser DEFAULT_VIEWPORT)
        user_agent          str              custom UA (rare — soulbrowser sets one already)
...
from soulbrowser import launch_context_async

logger = logging.getLogger("soulbrowser.lambda")
...
def _build_launch_kwargs(event: dict) -> dict:
    ...
    Only includes keys explicitly set in the event so soulbrowser's defaults
